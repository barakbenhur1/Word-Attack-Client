...
enum DifficultyType: String, Codable, CaseIterable {
    case ai = "⚔️ AI", pvp = "🌍 PVP" ,easy = "😀 Easy", medium = "😳 Medium", hard = "🥵 Hard", tutorial
        ...
        Button(action: action) {
            VStack(spacing: 6) {
                ...
                Text(title.localized)
                    ...
                    .foregroundStyle(isLocked ? .primary.opacity(0.55) : color)
...
/// Vertical "wheel" ScrollView:
/// - items loop infinitely by recentering in the middle block
/// - non-center items get parallax (scale / opacity)
/// - nearest row snaps to center
struct ParallaxVerticalScroll<Content: View>: View {
    let itemCount: Int
    let itemHeight: CGFloat
    let spacing: CGFloat
    let repeatCount: Int          // visual copies of the data (must be odd)
    @ViewBuilder let content: (Int) -> Content
    
    // used to debounce snapping so we don't fight with the user
    @State private var snapToken: Int = 0
    
    init(
        itemCount: Int,
        itemHeight: CGFloat = 72,
        spacing: CGFloat = 12,
        repeatCount: Int = 5,
        ...
        // ensure we have an odd number so there's a true "middle" block
        let base = max(repeatCount, 3)
        self.repeatCount = base.isMultiple(of: 2) ? base + 1 : base
    ...
    var body: some View {
        GeometryReader { outer in
            let viewHeight = outer.size.height
            let centerY = viewHeight * 0.5
            
            if itemCount == 0 {
                Color.clear
            } else {
                let totalRows = itemCount * repeatCount
                let middleBlock = repeatCount / 2
                let baseStart = middleBlock * itemCount
                let baseRange = baseStart ..< (baseStart + itemCount)
                
                ScrollViewReader { proxy in
                    ScrollView(.vertical, showsIndicators: false) {
                        LazyVStack(spacing: spacing) {
                            ForEach(0..<totalRows, id: \.self) { rawIndex in
                                GeometryReader { inner in
                                    let frame = inner.frame(in: .named("PARALLAX_SCROLL"))
                                    let cellMidY = frame.midY
                                    
                                    let distance = cellMidY - centerY
                                    let absDistance = abs(distance)
                                    let maxDistance = max(viewHeight / 2, 1)
                                    ...
                                    // only the centered-ish one is tappable
                                        .disabled(opacity < 0.97)
                                    // report this cell's midY for snapping / recentering
                                        .preference(
                                            key: CenterItemPreferenceKey.self,
                                            value: [rawIndex: cellMidY]
                                        )
                                }
...
//                    )
                    ...
                    .onPreferenceChange(CenterItemPreferenceKey.self) { midYByIndex in
                        guard !midYByIndex.isEmpty else { return }
                        
                        // which *row* is visually closest to the center?
                        guard let (nearestRow, nearestMidY) = midYByIndex.min(by: {
                            abs($0.value - centerY) < abs($1.value - centerY)
                        }) else { return }
                        
                        // INFINITE LOOP PART:
                        // if we drift into the top/bottom copies, warp back to middle block
                        if !baseRange.contains(nearestRow) {
                            let logicalRow = (nearestRow % itemCount + itemCount) % itemCount
                            let targetRow = baseStart + logicalRow
                            warp(toRow: targetRow, proxy: proxy)
                            return
                        }
                        
                        // snapping (within the middle block)
                        let distanceToCenter = abs(nearestMidY - centerY)
                        guard distanceToCenter > 1 else { return }
                        scheduleSnap(toRow: nearestRow, proxy: proxy)
                    }
                }
            }
        }
    ...
    // Debounced snap to nearest row in the MIDDLE block
    private func scheduleSnap(toRow row: Int, proxy: ScrollViewProxy) {
        let token = snapToken + 1
        snapToken = token
        
        Task { @MainActor in
            try? await Task.sleep(nanoseconds: 200_000_000)
            guard token == snapToken else { return }
    ...
    // Instant warp (no animation) to keep the user in the middle block
    private func warp(toRow row: Int, proxy: ScrollViewProxy) {
        // cancel any pending snap – this is a reposition, not a snap
        snapToken &+= 1
...
/// Reports each item's midY inside the scroll view so we can
/// find which one is closest to the visual center.
private struct CenterItemPreferenceKey: PreferenceKey {
