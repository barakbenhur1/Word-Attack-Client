...
struct AIGameView<VM: AIGameViewModel>: View {
    ...
    @State private var ai: AISrevice?
    ...
    private func closeView() {
        ...
        if vm.fatalError || ai == nil || ai!.solverState == .failed {
            router.navigateBack()
        } else {
    ...
    private func closeIfFalid() {
        if ai?.solverState == .failed { closeView() }
    }
    ...
    var body: some View {
        bodyContant()
            ...
            .onChange(of: ai?.closeViewOnFail, closeIfFalid)
            .onChange(of: ai?.showPhraseValue, handlePhrase)
    }
    
    @ViewBuilder private func bodyContant() -> some View {
        if let ai, ai.solverState == .ready { contant() }
        else { AIPackLoadingView(onCancel: closeView) }
    }
            ...
            ZStack {
                ...
                case .ai:
                    ...
                    WordView(
                        ...
                        current: $current,
                        length: length,
                        isSolved: .constant(!vm.wordValue.isEmpty && aiMatrix[i].joined().lowercased() == vm.wordValue.lowercased()),
...
struct EmptyCard: View {
    ...
    var borderColor: Color = .dynamicBlack.opacity(0.9)
...
struct Flip3D: AnimatableModifier {
    var degrees: Double
...
extension View {
    func boardFlip(isOther: Bool) -> some View {
        self.modifier(
            Flip3D(
                degrees: isOther ? 180 : 0,
                axis: (0,1,0),
                perspective: -1/850,
                lift: 18,
                blurMax: 1.3,
...
private extension View {
    func cardGlass(cornerRadius: CGFloat = 28) -> some View {
        self
            .padding(20)
            .background(
