...
private extension View {
    ...
    func themedText(_ tone: TextTone) -> some View {
        switch tone {
        case .primary:   return self.foregroundStyle(Color.dynamicBlack)                // crisp white
        case .secondary: return self.foregroundStyle(Color.dynamicBlack.opacity(0.78))  // soft white
        case .tertiary:  return self.foregroundStyle(Color.dynamicBlack.opacity(0.58))  // dimmer white
...
struct GameView<VM: DifficultyWordViewModel>: View {
    ...
    private func handleTimeAttackIfNeeded() {
        ...
        switch diffculty {
        case .tutorial: timeAttackAnimationDone = true
        default:
            guard endFetchAnimation else { return }
            guard vm.word.isTimeAttack else { timeAttackAnimationDone = true; return }
    ...
    init(diffculty: DifficultyType) {
        ...
        self.timeAttackAnimationDone = false
        ...
        ForEach(0..<rows, id: \.self) { i in
            ZStack {
                ...
                WordView(
                    ...
                    current: $current,
                    length: length,
                    isSolved: .constant(!vm.wordValue.isEmpty && matrix[i].joined().lowercased() == vm.wordValue.lowercased()),
                    word: $matrix[i],
                    gainFocus: gainFocus,
                    colors: $colors[i]) {
    ...
    @ViewBuilder private func timeAttackView(proxy: GeometryProxy) -> some View {
        ...
        .background(Color.dynamicWhite.opacity(0.6).ignoresSafeArea()) // darker veil over the glossy BG
    ...
    private func nextLine(i: Int)  {
        ...
        if guess.lowercased() == vm.word.word.value.lowercased() || i == rows - 1 {
            ...
            default:
                ...
                @inline(__always) func scoreAnimation() async {
        ...
        try? await Task.sleep(nanoseconds: 300_000_000)
            ...
            if value > 1 {
                ...
                for i in 1...value {
                    ...
                    await MainActor.run {
                        withAnimation {
                            vm.score = min(score + value, score + i)
                        }
                    }
                }
            } else {  await MainActor.run { vm.score = score + value } }
