...
class DeepLinker: Singleton {
    ...
    func preform() {
        ...
        Task(priority: .userInitiated) {
            await MainActor.run {
                switch url.deepLinkString {
                case let deepLinkString where deepLinkString.contains(.resume):     resume(url: url)
                case let deepLinkString where deepLinkString.contains(.invite):     invite(url: url)
...
private extension DeepLinker {
    ...
    func invite(url: URL) {
        ...
        } else if let host = url.host?.lowercased(), host == "wordzap.app" || host == "www.wordzap.app" {
...
private extension DeepLinker {
    func navigateTo(_ route: Router.Route) {
        Task {
            guard router.presented != route else { return }
...
extension URL {
    var deepLinkString: String { absoluteString }
}
